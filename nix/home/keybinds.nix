...
# overlay (nix/home/quickshell/qml/cheatsheet), reached with SUPER+/.
#
# Derived from nix/home/session/actions.nix instead of hand-curated: that
...
# config to draw arguments from, and nix/home/quickshell/tree.nix runs a
...
# `hl.gesture` (nix/home/hyprland.nix:345) and kitty's Shift+Enter bind from
# nix/home/kitty.nix:52 — neither is a `bind` entry, so neither belongs in
...
let
  ...
  findAction =
    ...
    in
    if matches == [ ] then
      throw "nix/home/keybinds.nix: references action `${name}`, which does not exist in nix/home/session/actions.nix"
    else
...
assert
  if uncovered != [ ] then
    throw "nix/home/keybinds.nix: keyed action(s) not covered by any cheatsheet row or collapse entry: ${
      builtins.concatStringsSep ", " (map (a: a.name) uncovered)
    }"
  else if overcovered != [ ] then
    throw "nix/home/keybinds.nix: keyed action(s) covered by more than one cheatsheet row or collapse entry: ${
