...
#
# The derivation is nix/packages/claude-desktop.nix, built at the flake level and handed
...
# so nothing is lost by pinning; see the bump recipe in nix/packages/claude-desktop.nix.
{
  claudeDesktop,
  config,
  lib,
...
in
{
  ...
  config = lib.mkIf cfg.enable {
    ...
    # nix/packages/claude-desktop.nix for why, and why no wrapper variable can change
