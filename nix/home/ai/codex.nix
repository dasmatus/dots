# Native port of nix/home/ai/claude.nix to OpenAI Codex CLI. The shared
...
# in nix/modules/system/core.nix.
{
  pkgs,
  lib,
  dots,
  ...
  searxng-mcp =
    pkgs.writers.writePython3Bin "searxng-mcp"
      ...
      ''
        ...
        # readOnlyHint defaults to false, meaning "this tool may change state", and plan
        # mode denies any MCP tool that says so, whatever the allow-list holds. Read-only is
...
in
{
  ...
  # (nix/system/hosts.nix services.ollama, gated on dots.ai.ollama), not a
  ...
  # the models. See the DynamicUser note in nix/system/hosts.nix.
