...
# rather than stylistic. `nix/home/keybinds.nix` is imported the same
...
# `nix/home/quickshell/tree.nix` runs a strict `builtins.toJSON` over the
# result — a lambda, a `pkgs` reference, or a store path anywhere in this file
# would break both. Commands are attached later, in `nix/home/session/default.nix`,
...
#
# Transcribed from `nix/home/hyprland.nix`: the five `hl.exec_cmd` calls in
...
let
  entries = [
    ...
    # The keybind cheatsheet (nix/home/keybinds.nix). A plain toggle now: the
  ...
  ++ [
    ...
    # nix/home/quickshell/qml/osd/Osd.qml.
    {
      name = "volume-mute";
      mods = [ ];
      key = "XF86AudioMute";
  ...
  # (flake/packages.nix, nix/home/quickshell/tree.nix), and one living only
...
if duplicateNames != [ ] then
  throw "nix/home/session/actions.nix: duplicate `name` value(s) in the action table: ${builtins.concatStringsSep ", " duplicateNames}"
else
  entries
