...
let
  ...
  # nix/home/desktop/quickshell/default.nix instead (see the module header above).
  execDefaults = {
    # daemons
  ...
  # nix/home/desktop/hyprland.nix:771 already leans on that same shell expansion, so
  ...
  commandDefaults =
    ...
    // {
      ...
      # (nix/home/desktop/hyprland.nix) is already `WantedBy=lock.target` and wires
    ...
    assertions = [
      {
        ...
        message = ''
          ...
          }. Every non-dispatch row in nix/home/desktop/session/actions.nix needs one or the other in nix/home/desktop/session/default.nix.
        '';
      }
      {
        assertion = danglingExecKeys == [ ];
        message = ''
          dots.session.exec has key(s) naming no action in nix/home/desktop/session/actions.nix: ${lib.concatStringsSep ", " danglingExecKeys}.
        '';
      }
    ];
