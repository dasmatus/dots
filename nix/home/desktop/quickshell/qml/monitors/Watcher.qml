// The monitor hotplug watcher is hyprmon.service's replacement, now living
...
// nix/home/desktop/quickshell/default.nix, the rename off ~/.config/hyprmon/
...
Scope {
    ...
    // `root` off it was silently undefined forever, and matchRule/matchOverride
    // never saw a real rule, so every hyprctl call this file exists to make
    ...
    // Startup, a FileView reload, a debounced Hyprland event, and this IPC
    // handler are each "the world may have changed, read again", and each
    // gets its own fresh retry budget: a read that is
    ...
    // `debounce` above uses) is a 1.5s budget. That's generous for a
    // compositor socket to appear, but bounded, because a compositor that is
    // genuinely absent must not spin this forever.
    ...
    // after every retry and never inside the same tick as the read it
