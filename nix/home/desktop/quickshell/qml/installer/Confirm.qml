// app.rs::Screen::Confirm. Renders Config.settingsNix(cfg) verbatim, the same
// writer Task 1 ported, so the user sees exactly what plan 3c would write,
// byte for byte, before typing the uppercase "ERASE" app.rs also requires. Esc
// returns to wherever Network sent the user on (Hostname if autodetection
// picked the disk, DiskSelect otherwise), matching app.rs's `self.screen =
// self.after_network()` on this exact key.
//
