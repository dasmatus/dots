...
# string nothing provides. That is the failure mode where the extension is
# installed, the dconf key looks right, and the Shell silently loads nothing.
{ pkgs, chromaleon, ... }:
let
  extensions =
    ...
    [
      ...
      # carries a "50" key. That is required, since this host runs GNOME Shell
      # 50 and an extension whose metadata.json omits the running version
      # installs cleanly and then never loads. Only about a third of that
      # revision's extensions declare 50 at all, so this is a real check
      ...
      # in the background" story is a tray icon simply has no way to stay
      # reachable with its window closed once its main window is gone. That
      # covers the mail client, the vault client and the chat clients, and
      # makes this the load-bearing extension for the flatpak set in
