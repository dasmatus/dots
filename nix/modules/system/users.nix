...
# flow (git history); the username comes from nix/data/settings.nix (written by
# the installer TUI).
...
let
  ...
  # clone (nix/home/base/dots-repo.nix only copies settings.nix + facter.json). At
...
in
{
  ...
  # of the two prerequisites `system.nixos-init` (nix/modules/system/core.nix)
