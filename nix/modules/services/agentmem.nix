...
#                          agentmem_mcp, and nix/modules/services/agentmem/migrations/.
#
# Gated on the same flag as the plugin it serves (nix/home/ai/claude.nix): the
...
in
{
  config = lib.mkIf config.dots.ai.claude {
    services.postgresql = {
      ...
      # socket (nix/system/hosts.nix rules out DynamicUser and ReadWritePaths here
    ...
    # nix/modules/services/agentmem/migrations/*.sql already applied, by filename, so a
