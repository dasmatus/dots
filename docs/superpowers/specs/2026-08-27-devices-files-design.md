...
`systemd.user` automount unit, which gives no route back into the shell's
own state, so the bar pill and the sidebar would have nothing to read.

**Filter on `hotplug`, never on `rm`, and never on fstype alone.** Measured
on this machine: `/dev/sda1` (the attached WD 2TB USB disk) reports
`rm: false, hotplug: true`. A USB hard disk is not "removable media" in the
kernel's own `rm` sense, so filtering on `rm` would silently drop the disk
the user actually plugged in, and `/dev/sda1` staying in the list despite
its own `rm: false` is what proves `hotplug` is the right column to key on,
not `rm`. `/dev/nvme0n1p1`, the EFI System Partition, reports
`fstype: "vfat", mountpoint: "/boot", hotplug: false`. It carries an fstype
like any real drive, so with no `hotplug` clause at all it would still pass
every other check and land in the device list, sitting in the bar pill and
the sidebar next to the real USB disk though it is the boot disk's own
partition. Excluding it is what proves the `hotplug` clause exists at all,
a separate question from which column that clause should key on. The
captured fixture at
`.superpowers/sdd/rosy-zooming-lemon/lsblk-fixture.json` carries both
devices side by side, so both checks are unit tests, not a hope.
...
only for its exit code, and the resulting mount state is always re-read
from a fresh `lsblk`, never parsed from what `udisksctl mount` printed.
...
**`eject()`'s disk lookup needs no `PKNAME` column and no reuse of
`qml/installer/disks.js`'s `parentDisk()`.** Finding the whole disk behind a
partition is a problem the installer's own `disks.js` already solved once,
by stripping a trailing partition suffix (`/dev/sda1` → `/dev/sda`,
`/dev/nvme0n1p2` → `/dev/nvme0n1`) as a fallback for when a `PKNAME` lookup
comes back empty. `devices.js` needs neither half of that: its `lsblk` walk
already descends from each top-level disk to its partitions, so every
parsed device carries its own `diskPath`, fixed at the top of that disk's
subtree and inherited unchanged by every descendant. `ejectPlan(devices,
diskPath)` reads that field straight off the record, unmounts every device
still mounted on that disk, and appends the `power-off` step, correctly
covering the case a suffix-stripping heuristic never had to: a childless
"disk" carrying a filesystem directly, with no partition suffix to strip
and no `PKNAME` of its own to look up.
...
an argument.** Every `IpcHandler` function in this tree today takes zero
arguments: `launcher`'s `open`/`close`/`toggle`, `wallpaper`'s
`open`/`close`/`toggle`, `monitors`' `apply`, `arrange`'s
`open`/`close`/`toggle`, `settings`'s `open`/`close`/`toggle`, `cheatsheet`'s
`toggle`/`close`. There is exactly one exception, `wallpaper`'s
...
process, a keybind or a script, is a real future caller, and that gap gets
...
## Architecture
    ...
    │   └── devices.js          parseDevices/mountCandidates/pruneAttempts/newlyMounted/displayLabel/ejectPlan
    ├── bar/
...
`lsblk -J -b -o NAME,PATH,LABEL,SIZE,FSTYPE,MOUNTPOINT,HOTPLUG,TYPE,VENDOR,MODEL`,
hands the raw JSON text to `devices.js`'s `parseDevices()`, which already
returns only hotplug partitions and hotplug superfloppy disks carrying a
filesystem, keyed with each device's own `diskPath`. `mountCandidates(
devices, attempted)` narrows that to `!mountPoint`, not already the target
of a mount this singleton started; each candidate is marked in `attempted`
before `udisksctl mount -b <path>` runs, exit code only, followed
immediately by another rescan to pick up the real mountpoint udisks2 chose.
`pruneAttempts()` clears an `attempted` entry only once its path drops out
of a scan entirely, i.e. unplugged, so a device that merely fails to mount
is offered once and then left alone instead of retried forever.
`newlyMounted()` diffs consecutive scans of `devices` (the property every
other consumer reads, `flat` narrowed to `mountPoint !== null`) and fires a
`notify-send` toast for each transition, each device carrying a `label`
computed by `displayLabel()` (the device's own filesystem label, falling
back to the trimmed vendor and model string, falling back to the device
name). `eject(path)` looks up the clicked device's own `diskPath` and hands
it to `ejectPlan(devices, diskPath)`, which unmounts every device still
mounted on that disk before the single `udisksctl power-off`, which is
what actually cuts power to a USB device so it is safe to physically pull,
not merely `unmount`.
...
- QtTest in `tests/qml/` for every pure `.pragma library`: `tst_devices.qml`
  against the real captured `lsblk-fixture.json`. `test_efi_system_partition_is_excluded`
  is load-bearing for the claim that a hotplug clause exists at all: delete
  it and the ESP, mounted at `/boot` but hotplug false, lands in the device
  list. It is not what proves `hotplug` over `rm`, since the ESP reports
  `rm: false` too, and swapping one column for the other there still
  passes. `test_real_usb_disk_with_rm_false_is_included` is what proves
  that: `/dev/sda` and `/dev/sda1` both report `rm: false`, so an rm-based
  filter would wrongly drop the user's actual USB disk (see Plan 0).
  `tst_files.qml` covers `parseListing`/`join`/`parentOf`,
  `tst_files_operations.qml` covers the four argv builders' shape.
