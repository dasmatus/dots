...
import Reflex.Vty.ColorProfile (ColorProfile (..))
...
import Reflex.Vty.Widget.Layout (initManager_)

import AbstractTUI.Base.Geom (Size (..))
import AbstractTUI.Reactive (Scope, TriggerFactory (..), drainScopeTriggers, newScope)
import AbstractTUI.Term (CaptureTerm (..), captureDrainInput)
import AbstractTUI.View (BuildEnv, View (..), unView)
...
widget monad *outside* 'initManager_' (see 'vtyGuest'), in the 'BuildEnv'
capability region — which lifts 'MonadRef' from the Spider host so
'AbstractTUI.Reactive.signal' can call 'newEventWithTriggerRef'. It builds
the 'View' tree (pure data + signal allocation) and does NOT lay out
children; 'unView' does the layout inside 'initManager_'.
-}
...
newDriver app term cfg =
    (runSpiderHost :: SpiderHost Global Driver -> IO Driver) $ do
        ...
        chan <- liftIO newChan
        -- The 'TriggerFactory' 'signal' uses to allocate 'Signal' events from
        -- inside the widget-stack closure (which lacks 'MonadRef'). It is a
        -- nested 'runSpiderHost' call (invoked via 'liftIO' in 'signal') that
        -- only /allocates/ an event + trigger-ref — no frame processing — which
        -- is safe under Reflex's shared 'Global' timeline. The type signature on
        -- 'triggerIO' drives polymorphism over the signal element type @a@ so
        -- one factory serves signals of any type.
        let triggerIO :: forall a. IO (Event Spider a, IORef (Maybe (EventTrigger Spider a)))
            triggerIO = runSpiderHost newEventWithTriggerRef
        scope' <- liftIO (newScope (TriggerFactory triggerIO))
        displayRegion0 <-
...
{- | The reflex-vty guest: set up the full widget-context stack (matching
'Reflex.Vty.Widget.mainWidgetWithHandle') and run the app's view-builder as
the child widget, wrapped in 'initManager_' so the 'Layout'\/'Focus'
monads ('HasLayout'\/'HasFocus') are available to the view. The picture is
@picForLayers (reverse images)@ — the background fill is told first, the
child's images on top.

The stack is: 'ThemeReader' → 'ColorProfileReader' → 'FocusReader' →
'DisplayRegion' → 'ScreenModeWriter' → 'CursorWriter' → 'ImageWriter' →
'NodeIdT' → 'Input'. The fixed 'ColorProfile_TrueColor' keeps true-color
'V.Attr's intact for the capture harness (a real tty would downsample via
'colorProfileFromVty', but the mock output reports no color capability).
Cursor / screen-mode writer events are collected (so widgets that
'tellCursor'\/'tellScreenMode' compile and don't escape) but, for the
capture driver, not applied to a real terminal; they are folded into the
picture only if a future bin needs a visible cursor.
-}
vtyGuest ::
    forall t m.
    (MonadVtyApp t m) =>
    ...
    m (VtyResult t)
vtyGuest dr0 vtyEvent signalEvent scope viewBuilder = do
    -- Fixed display size (no resize handling); the size stays at the capture
    -- terminal's initial bounds for the lifetime of the app.
    size <- holdDyn dr0 never
    let inp' = vtyEvent
        profile = ColorProfile_TrueColor
        sigShutdown =
            ()
                <$ ffilter
                    (\s -> s == AppSignal_Interrupt || s == AppSignal_Terminate || s == AppSignal_Hangup)
                    signalEvent
    ((((shutdown, images), _cursorUpdates), _screenModeUpdates)) <-
        runThemeReader (pure defTheme) $
            runColorProfileReader (pure profile) $
                                ...
                                runImageWriter $
                                    runNodeIdT $
                                        runInput inp' $ do
                                            ...
                                            -- Build the View tree OUTSIDE 'initManager_': this region
                                            -- lifts 'MonadRef' from the Spider host, so 'signal' can
                                            -- call 'newEventWithTriggerRef' and obtain a trigger ref
                                            -- for synchronous 'fireEventRef'. The tree is pure data
                                            -- (signal allocation + 'Element' records); no layout yet.
                                            view <- viewBuilder scope
                                            -- Run the tree INSIDE 'initManager_', where 'Layout'/
                                            -- 'Focus' supply 'HasLayout'/'HasFocus' for 'axis'/'grout'.
                                            initManager_ (unView view)
    pure
        VtyResult
            { _vtyResult_picture =
...
2. Fire any input events queued by 'feedInput' (Task 9): drain 'ctInput'
   ...
   (e.g. 'quitterQuit', and the 'onEvent' handlers that 'sigSetIO'/
   'sigUpdateIO' queue a 'Signal' mutation) run synchronously within each
   'fire' (via 'hostPerformEventT'), so 'quitterCheck' below observes
   their effect and the 'Signal' queues are populated before step 3.
3. Drain IO-queued 'Signal' mutations: 'drainScopeTriggers' collects the
   latest queued value per 'Signal' as 'EventTrigger' assignments and we
   fire them through the host's 'FireCommand' — the same path
   'Reflex.Vty.Host' 'fireEventTriggerRefs' uses — which drives a full
   'hostPerformEventT' frame so 'holdDyn' propagates to the 'Dynamic'
   /and/ the downstream push-'Behavior's ('tellImages' \/ the picture)
   refresh. ('fireEventRef' is deliberately not used here: it fires the
   trigger but skips the frame, so the 'BehaviorWriter' merge that feeds
   the picture would keep its last-frame cache and the mutation would not
   land same-turn.)
4. Render: sample the 'Picture' behavior and call 'V.update' (line
   200-201), which records the picture and appends this frame's emitted
   bytes to 'ctEmitted' on the capture terminal.
5. Compute 'turnEmitted' by comparing 'ctEmitted' length before/after
...
6. Read the quit flag for 'turnQuit'.
-}
turn :: Driver -> IO Turn
turn dr =
    (runSpiderHost :: SpiderHost Global Turn -> IO Turn) $ do
        ...
        -- Drain IO-queued 'Signal' updates produced by the handlers above (which
        -- ran in 'performEvent_' ~ IO and could only 'sigSetIO'/'sigUpdateIO' into
        -- the per-Signal queue). 'drainScopeTriggers' collects the latest queued
        -- value per signal as an 'EventTrigger' assignment; we fire them through
        -- the host's 'FireCommand' (the same path reflex-vty's own loop uses —
        -- 'Reflex.Vty.Host' 'fireEventTriggerRefs'), which drives a full
        -- 'hostPerformEventT' frame so 'holdDyn' propagates to the 'Dynamic' /and/
        -- the downstream push-'Behavior's ('tellImages' \/ the picture) refresh
        -- before the render below samples the picture. 'fireEventRef' is NOT used
        -- here: it fires the trigger but skips the frame, leaving the
        -- 'BehaviorWriter' merge on its last-frame cache (the picture would not
        -- update same-turn). This is the same-turn bridge from IO-handler signal
        -- mutation to the picture.
        pending <- liftIO (drainScopeTriggers (drScope dr))
        unless (null pending) $ void (fire pending (return ()))
