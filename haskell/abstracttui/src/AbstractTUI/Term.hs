...
'captureCell' composites the 'Picture's 'Image' layers (top-most first,
as 'picForLayers' stores them) into a (x,y) -> 'Char' grid by recursing
through the 'Image' constructors (see "Graphics.Vty.Image.Internal").
Under the default 'ClearBackground' a layer's 'BGFill' / uncovered cells
are transparent — they show the layer below — so 'captureCell' returns the
first opaque cell scanning top-to-bottom, else the background space.
'captureEmit' drains the accumulated
...
every layer is transparent there (background). The 'Picture's layers are
scanned top-to-bottom (the order 'picForLayers' stores them); under the
default 'ClearBackground' a layer's 'BGFill' / uncovered cells are
transparent and fall through to the layer below, so 'captureCell' returns
the first opaque cell found, or ' ' if none. See 'cellChar' for what
counts as opaque versus transparent.

This is in 'IO' (not pure like the Rust @capture_cell@, which takes
@&self@ and reads a pre-settled picture under an immutable borrow): the
Haskell 'CaptureTerm' stores the picture in a mutable 'IORef' the mock
'V.Vty' 'update' settles each 'turn', and a pure read via
'unsafePerformIO' is unsound here — GHC's full-laziness floats the
@captureCell term x y@ application out of a test's 'do'-block and shares
the thunk across call sites, so a second call (after a later 'turn' has
settled a new picture) would return the first call's memoized 'Char'.
Keeping it in 'IO' sequences each read after the 'turn' that settles it.
-}
captureCell :: CaptureTerm -> Int -> Int -> IO Char
captureCell term x y = do
    mpic <- readIORef (ctPicture term)
    case mpic of
        Nothing -> pure ' '
        Just pic -> pure (compositeCell (picLayers pic) x y)

{- | Composite a layer stack top-to-bottom: the first layer that yields an
opaque cell at @(x, y)@ wins; if all are transparent (or the stack is
empty) the background space is returned. Matches vty's 'ClearBackground'
rendering, where a layer's 'BGFill' / out-of-bounds cells show the layer
below rather than occluding it.
-}
compositeCell :: [Image] -> Int -> Int -> Char
compositeCell layers x y = go layers
  where
    go [] = ' '
    go (l : ls) =
        case cellChar l x y of
            Just c -> c
            Nothing -> go ls
...
{- | Flatten an 'Image' to the opaque character at @(x, y)@, or 'Nothing'
if the cell is transparent (background). 'Nothing' covers out-of-bounds
cells, 'BGFill', and 'EmptyImage' — all transparent under vty's
'ClearBackground' — so a 'Just' is a real foreground cell (a 'HorizText'
character, including a literal space in the text, which is opaque and
occludes the layer below). The 'Image' constructors live in
"Graphics.Vty.Image.Internal" (the public "Graphics.Vty.Image" module
exports 'Image' abstractly); we qualify the constructor names to keep
the import list unambiguous.
-}
cellChar :: Image -> Int -> Int -> Maybe Char
cellChar img x y = case img of
    V.Image.HorizText _ txt _ _ ->
        let charLen = fromIntegral (TL.length txt) :: Int
         in if y == 0 && x >= 0 && x < charLen
                then Just (TL.index txt (fromIntegral x))
                else Nothing
    V.Image.HorizJoin l r _ _ ->
        let lw = imageWidth l
         in if x < lw then cellChar l x y else cellChar r (x - lw) y
    V.Image.VertJoin t b _ _ ->
        let th = imageHeight t
         in if y < th then cellChar t x y else cellChar b x (y - th)
    V.Image.BGFill _ _ -> Nothing
    V.Image.Crop i ls ts ow oh
        | x >= 0 && x < ow && y >= 0 && y < oh -> cellChar i (x + ls) (y + ts)
        | otherwise -> Nothing
    V.Image.EmptyImage -> Nothing
