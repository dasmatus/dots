...
```
nix run .#nix-smoke                    # = nix build -L .#checks.x86_64-linux.iso-secureboot
nix run .#nix-smoke -- --no-secure-boot   # = nix build -L .#checks.x86_64-linux.iso-boot
...
`nix run .#nix-smoke-interactive` drops you into the driver's Python REPL with
...
- **`signedIso` fixture** — runs `scripts/sign-iso.sh` (the same script
  `nix run .#iso` uses; single source of signing truth) inside the build
