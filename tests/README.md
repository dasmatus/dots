...
| `sandbox-machined` | eval-only, no VM, no build (see [`sandbox-machined.nix`](sandbox-machined.nix)): a standalone home-manager evaluation of `nix/home/sandbox/machined.nix` — the user-scope `systemd-machined` units the per-app sandbox needs (`machinectl --user` with no polkit prompt) are actually placed, every one of them is a `pkgs.systemd`-store-path reference rather than hand-copied unit text, the socket carries its `sockets.target.wants` enablement symlink, and the unused `systemd-nspawn@`/`systemd-vmspawn@` user templates plus `machines.target` stay unwired |
