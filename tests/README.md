...
| `limine-install-home` | `nix/modules/limine-install.nix`'s HOME-provisioning fix (see [`limine-home.nix`](limine-home.nix)): with `$HOME` unset, or set to a directory a different user owns, the wrapper invokes the real `mktemp` binary and exports the owned temp dir it prints; with `$HOME` already owned by the caller, the wrapper leaves it alone |
