...
let
  hm = inputs.home-manager.lib.homeManagerConfiguration {
    inherit pkgs;
    # `dots.ai.ollama` is the one field nix/home/apps/zed.nix reads (whether to
    ...
    # and nix/home/desktop/hyprland.nix and nix/home/desktop/session/default.nix no longer
    # reference either.
    extraSpecialArgs = {
  ...
  actions = import ../nix/home/desktop/session/actions.nix;
  ...
  # indirection nix/home/desktop/session/default.nix's `commands` exists to
  ...
  # belong to the WM module (nix/home/desktop/hyprland.nix's own `env` list), never
  ...
  # the wallpaper-accent retint in nix/home/desktop/quickshell/qml/wallpaper/Kvantum.qml.
  ...
  # `dots-screenshot-*` unit (nix/home/desktop/hyprland.nix's job to set) is what
...
assert lib.assertMsg (stranded == [ ])
  "tests/session-units.nix: action(s) with neither a unit nor a dots.session.commands entry: ${strandedMsg}. Every non-dispatch row in nix/home/desktop/session/actions.nix needs one or the other.";
...
assert lib.assertMsg (hyprlandStartLines == [ ]) ''
  ...
  ${hyprlandStartMsg}
  Every daemon/startup action is a systemd unit now (nix/home/desktop/session/default.nix); nothing should launch from this hook any more.'';
...
assert lib.assertMsg (wmOwnedVarsPresent == [ ])
  "tests/session-units.nix: systemd.user.sessionVariables carries WM-owned variable(s) that belong only to nix/home/desktop/hyprland.nix: ${wmOwnedVarsPresentMsg}.";
