...
//! `pkexec`, no `sudo`, ever, for any of them — and every external command
...
// ---------------------------------------------------------------------
// 24h capability timeline (the sandbox's own audit log)
// ---------------------------------------------------------------------
...
/// Cap the timeline at this many of the most recent lines, so a long-lived
/// machine's audit log cannot blow up the report's JSON size; the QML
/// page is meant to show a recent-activity feed, not a full history.
const TIMELINE_LIMIT: usize = 200;

/// Parses the audit log's JSON Lines format (one object per line, per the
/// launcher task's schema) into a 24h activity timeline, newest first.
/// Malformed lines are skipped individually rather than failing the whole
/// card — a single corrupted line must not hide every other app's
/// history. `now_ms` is injected so the 24h window is testable without
/// depending on the real clock.
#[must_use]
pub fn parse_audit_timeline(jsonl: &str, now_ms: u128) -> Card {
    let mut lines: Vec<AuditLine> = jsonl
        .lines()
        .filter(|line| !line.trim().is_empty())
        .filter_map(|line| serde_json::from_str::<AuditLine>(line).ok())
        .filter(|line| now_ms.saturating_sub(line.ts_ms) <= DAY_MS)
        .collect();
    lines.sort_by_key(|line| std::cmp::Reverse(line.ts_ms));
    lines.truncate(TIMELINE_LIMIT);
    ...
    let rows = lines
        ...
        .map(|line| {
            let what = line
                .capability
                .as_deref()
                .map_or_else(|| line.kind.clone(), |cap| format!("{}:{cap}", line.kind));
            let value = line.detail.as_deref().map_or_else(
                || format!("{} -> {}", what, line.outcome),
                |detail| format!("{} -> {} ({detail})", what, line.outcome),
            );
            Row::new(line.app_id.clone(), value)
        })
        .collect::<Vec<_>>();

    Card {
        id: "audit_timeline",
        title: "24h capability activity",
        status: Status::Ok,
        detail: format!("{} event(s) in the last 24h", rows.len()),
        rows,
    }
}

fn audit_timeline_card() -> Card {
    let path = crate::broker::AuditLog::open_default().path().to_path_buf();
    match std::fs::read_to_string(&path) {
        Ok(jsonl) => parse_audit_timeline(&jsonl, now_ms()),
        Err(err) if err.kind() == io::ErrorKind::NotFound => Card {
            id: "audit_timeline",
            title: "24h capability activity",
            status: Status::Ok,
            // The honest, expected state on a machine where nothing has
            // ever launched through the sandbox yet — degrading cleanly
            // here, not failing, is the whole point of a JSONL log that
            // simply doesn't exist until the first app runs.
            detail: "no sandboxed app has logged any activity yet".to_string(),
            rows: Vec::new(),
        },
        Err(err) => Card::unavailable(
            "audit_timeline",
            "24h capability activity",
