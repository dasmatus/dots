...
pub fn decide(
    ...
    let outcome = match (state, interactivity) {
        (CapState::Allow, _) => Outcome::AllowedByPolicy,
        (CapState::Deny, _) => Outcome::DeniedByPolicy,
        (CapState::Ask, Interactivity::NonInteractive) => Outcome::DeniedNonInteractive,
        (CapState::Ask, Interactivity::Interactive) => prompt(capability),
    };
