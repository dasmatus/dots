//! Full-redraw contract for the wallpaper TUI's custom `Driver` loop.
//!
...
//! desync (an external `clear`, emulator glitch, scrollback bleed) — and any
//! lost preview-image placement — therefore self-heals on the next draw.
//!
...
/// An app over an empty wallpaper folder — enough to mount `root_view` and
/// paint the "No wallpapers found" screen; the screen content is irrelevant,
/// only the redraw contract is under test.
fn empty_app() -> App {
...
fn request_full_redraw_re_emits_an_unchanged_frame() {
    ...
    // re-emits every cell (and re-places the preview image) this frame.
