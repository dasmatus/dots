...
//! priority over the abstraction table both by ordering, since every Block
//! rule runs first, and by omission, since an abstraction that grants what
//! this repo blocks is simply never a table entry. See the comment above
//! `ABSTRACTIONS` for the full argument.
//!
...
//! guess. That is the same stance `report` takes when it answers `Unavailable`
...
pub struct Classification {
    ...
    /// "nameservice". `None` on every Block, since a block is never softened
...
pub struct TriageCtx {
    ...
    /// Extra paths to treat as always-blocked, on top of the
    /// [`CREDENTIAL_DIRS`]/[`SENSITIVE_DEVICES`]/[`SENSITIVE_ETC`] tables
    /// below. Used to come from the per-app sandbox's own resolved policy
    /// (`policy::resolve_all`, deleted along with the rest of the sandbox —
    /// see git history); the CLI wiring this crate ships with passes an
    /// empty list, since there is no longer a per-app policy to read one
    /// from. Left as a field, not removed, because a future caller with its
    /// own denylist (an AppArmor profile's own hand-written `deny` rules,
    /// say) still has somewhere to plug it in without touching the
    /// classifier.
    pub deny_paths: Vec<PathBuf>,
    /// Known app ids, used to decide whether `~/.config/<x>` is the app's own
    /// state or somebody else's. Same story as `deny_paths`: the CLI wiring
    /// passes an empty list, which only widens what falls through to
    /// [`Verdict::Unclassified`] rather than misclassifying anything — the
    /// safe direction for an empty hint.
...
enum Anchor {
    ...
    /// `wayland-[0-9]*` glob exactly: one digit, then anything at all. So
    /// `wayland-1` and `wayland-0extra` both match, same as upstream,
...
/// That is the same reasoning `argv.rs`'s `push_gui_binds` already gives
/// for never binding it into a sandbox in the first place. A denial here
/// means "let me leave the confinement", not "grant me one more path".
...
/// `r`, `m` and `k` read, map and lock. Anything else mutates or executes
/// and is not covered by the read-only rule: `w`, `a`, `c`, `d`, `x`, `l`.
...
pub fn denial_from_journal_line(line: &str) -> Option<Denial> {
    ...
    // parses fine and simply is not a denial, such as every
    // `apparmor="STATUS"` profile-load, of which there are hundreds after a
    // rebuild, returns `None` silently, because logging those would drown
...
/// rather than derived, matching this crate's choice not to pull in
/// `thiserror`.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Diagnostic)]
pub enum QueryRejection {
    /// Contains a path separator, in any form.
    #[diagnostic(
        code(dots_secreport::triage::query_is_a_path),
        help(
    ...
    #[diagnostic(
        code(dots_secreport::triage::query_names_the_user),
        help("strip the username and search for the generic term on its own")
    )]
    NamesTheUser,

    /// Nothing left to search for.
    #[diagnostic(code(dots_secreport::triage::query_empty))]
    Empty,
}
...
/// The whole document `dots-secreport triage --json` prints.
#[derive(Debug, Clone, Serialize)]
